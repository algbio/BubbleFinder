...
methods (a brute-force tool and BubbleFinder), and compares their outputs.
...
def run_snarls_bf(bruteforce_bin, gfa_path):
    ...
    return parse_snarls_bruteforce_output(res.stdout)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Max number of failing graphs whose full GFA is printed in the final report"
    )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if args.keep_failing:
                    ...
                    dest_stdout.write_text(e.stdout or "")
            ...
            # Compare between different thread configs (determinism / parallelism issues)
    ...
    print(f"\nDetails for {len(sample)} failing graph(s) (sample):")
    for f in sample:
        print("\n------------------------------")
